...
import { Ticket as TicketIcon, AlertTriangle, MessageCircle } from "lucide-react"
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    CANCELED: "bg-amber-100 text-amber-700 border-amber-200", // Tambahan handle typo database
  }
...
export default async function TicketsPage({
  ...
  // 2. TAMBAHKAN LOGIKA FILTER STATUS
  if (status && status !== "all") {
    // Mengonversi "in-progress" dari URL menjadi "IN_PROGRESS" untuk database
    ...
    <section className="flex flex-1 flex-col gap-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
      ...
      <div className="grid gap-4">
          {allTickets.length > 0 ? (
            ...
            // Tampilan jika tidak ada tiket sama sekali
