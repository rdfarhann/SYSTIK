    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-mono bg-slate-100 px-2 py-1 rounded text-slate-500">
            ID: #{String(ticket.id).slice(0, 8)}
          </span>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-slate-500">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold uppercase">Priority: {ticket.priority}</span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-8 space-y-5">
          <Card className="rounded-2xl border-slate-200 shadow-sm overflow-hidden bg-white">
            <div className="p-5 space-y-4">
              ...
              <div className="text-slate-600 leading-relaxed whitespace-pre-wrap text-sm border-l-2 border-primary/20 pl-4 italic">
                {ticket.description}
              </div>
                ...
                <div className="pt-2">
                  <div className="flex items-center justify-between p-2 rounded-xl border border-slate-100 bg-slate-50 group transition-colors">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="icon" asChild className="h-8 w-8 text-foreground hover:text-background hover:bg-primary">
                        <a href={ticket.attachment_url} download>
        ...
        <div className="lg:col-span-4 space-y-5">
          <Card className="p-5 rounded-2xl border-slate-200 shadow-sm bg-white">
            ...
            {!(["CLOSED", "CANCELED"].includes(ticket.status)) && ticket.sla_deadline ? (
              ...
              <div className="mb-6 p-3 rounded-xl bg-slate-50 border border-dashed border-slate-200 flex items-center gap-3">
            ...
            <div className="relative pl-6 space-y-5 before:absolute before:inset-0 before:left-[7px] before:h-full before:w-[1.5px] before:bg-slate-100">
              {logs.length > 0 ? logs.slice(0, 5).map((log: TicketLog, index: number) => (
                <div key={log.id} className="relative">
                  <div className={`absolute -left-[22px] mt-1.5 h-2.5 w-2.5 rounded-full border-2 border-white ${index === 0 ? "bg-primary" : "bg-slate-300"
                    }`} />
