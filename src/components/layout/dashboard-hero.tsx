...
// Pastikan path import ini sesuai dengan lokasi file AllTicketsTable Anda
...
export default function DashboardHero() {
  return (
    <div className="space-y-6 w-full max-w-full overflow-x-hidden px-1 sm:px-0">
      ...
      {/* ================= TICKET LIST TABLE (Memanggil Komponen AllTicketsTable) ================= */}
...
function StatCard({ title, value, description, icon: Icon, spin = false }: { title: string, value: string, description: string, icon: LucideIcon, spin?: boolean }) {
  return (
    <Card className="rounded-xl p-3 sm:p-4 transition-all hover:scale-[1.03] bg-background text-foreground border-foreground shadow-md group hover:rotate-1 hover:bg-primary hover:text-background">
